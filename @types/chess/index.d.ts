...
declare module "chess" {
  namespace Chess {
    ...
    interface GameClient extends GameStatus {
      ...
      /** Add event listeners */
      on(event: MoveChessEvent, cbk: (move: Move) => void): void;
      on(event: AttackChessEvent, cbk: (attack: AttackEventData) => void): void;
      on(event: PromoteChessEvent, cbk: (square: Square) => void): void;
    ...
    type File = string;
    type Rank = number;
    type AttackChessEvent = "check" | "checkmate";

    type MoveChessEvent = "capture" | "castle" | "enPassant" | "move";

    type PromoteChessEvent = "promote";
